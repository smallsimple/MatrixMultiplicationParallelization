...
__global__ void p1_kernel(double *A, double *B, double *C) {
  ...
  int N2=N*N;
	...
	__shared__ double As0[bSizeX][N];
	__shared__ double Bs0[bSizeX][N];
	__shared__ double As1[bSizeX][N];
	__shared__ double Bs1[bSizeX][N]; 

	double Csub0 = C[k*N+l];
	double Csub1 = C[(k+1)*N+l];
	for (int i=0; i<N; i++)
		for(int j=0; j<N; j++)
			Csub0 += 0.5*(A[(k+1)*N2+i*N+j]*B[k*N2+i*N+j]+A[k*N2+i*N+j]*B[(k+1)*N2+i*N+j]);
	for (int is=0; is<N; is+= bSizeX){	
		for (int j=0; j<N; j++){
			As0[tx][j] = A[k*N2+(is+tx)*N+j];
			Bs0[tx][j] = B[k*N2+(is+tx)*N+j];
			As1[tx][j] = A[(k+1)*N2+(is+tx)*N+j];
			Bs1[tx][j] = B[(k+1)*N2+(is+tx)*N+j];
		}
		__syncthreads();
		if(k<l-1){
		        for (int i=is; i<is+bSizeX; i++)
				for (int j=0; j<N; j++){
        			Csub0+=0.5*(As0[i-is][j]*B[l*N2+i*N+j]+A[l*N2+i*N+j]*Bs0[i-is][j]);
  					Csub1+=0.5*(As1[i-is][j]*B[l*N2+i*N+j]+A[l*N2+i*N+j]*Bs1[i-is][j]);
				}
		}
  	__syncthreads();
...
void compare()
{
    ...
    for (i=0;i<N;i++)
        for (j=0;j<N;j++)
        {
        	...
        	printf("i=%3d  j=%3d  Refout=%5.5lf  C=%5.5lf   diff=%5.5lf\n",i,j,RefOut[i*N+j],C[i*N+j],this_diff);
	    }
        }
    if (numdiffs > 0)
