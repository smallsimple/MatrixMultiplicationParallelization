...
int main (int argc, char *argv[])
{
   ...
   printf("id=%d n=%lld p=%lld      %lld  %lld  %lld\n",id,n,p,id,id*(n-1),id*(n-1)/p);
   //printf("low_value=%lld      %lld, id:%d n:%d p:%d\n", low_value, id*(n-1)/p,id,n,p);
   high_value = 1 + (id+1)*(n-1)/p;
   size = high_value - low_value + 1;
   printf("id=%2d low_value=%10d, high_value=%10d, size=%10d\n\n",id,low_value,high_value,size);
